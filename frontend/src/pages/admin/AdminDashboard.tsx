...
import { IndianRupee, Users, TrendingUp, ArrowUpRight, ArrowDownRight, Loader2 } from "lucide-react";
import { StatCard } from "@/components/shared/StatCard";
import { PageHeader } from "@/components/shared/PageHeader";
import { useAuth } from "@/context/AuthContext";
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from "recharts";
import { adminService, peopleService } from "@/services/apiServices";
...
export default function AdminDashboard() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [summaryData, transactions, people] = await Promise.all([
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Global financial overview across all users"
      />
      ...
      >
        <StatCard
          ...
          icon={IndianRupee}
          trend={{ value: "All users", positive: false }}
        />
        <StatCard
          title="Net Balance"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        >
          <h3 className="text-sm font-medium mb-4">Organization Performance</h3>
          <div className="h-64">
        ...
        >
          <h3 className="text-sm font-medium mb-4">Latest Transactions</h3>
            ...
            {recent.map((t) => (
              <div key={t.id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-muted/50 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">{t.peopleName} · {t.categoryName}</p>
                </div>
